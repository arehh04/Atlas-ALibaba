  ...
  <div class="ops-card p-3 sm:p-4 flex flex-col h-full font-mono text-xs">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-warm-200 pb-3 mb-3 gap-2">
      ...
      <div class="flex flex-wrap items-center gap-1.5 sm:gap-2 text-xs">
        ...
        <input v-model="searchQuery" type="text" placeholder="Filter..." class="bg-white border border-warm-200 rounded-xl px-2.5 py-1.5 sm:py-1 text-[11px] text-warm-800 placeholder-warm-400 focus:outline-none focus:border-brand-purple/40 w-full sm:w-36 transition-all" />
        <label class="flex items-center gap-1.5 text-warm-500 cursor-pointer select-none text-[11px]">
          <input type="checkbox" v-model="autoScroll" class="rounded bg-white border-warm-300 text-brand-purple focus:ring-0">
          <span>Auto</span>
        </label>
    ...
    <!-- Terminal Output -->
    <div ref="terminalWindow" class="flex-1 overflow-y-auto rounded-2xl bg-warm-900 border border-warm-700 p-3.5 space-y-1.5 text-xs leading-relaxed scanline-overlay">
